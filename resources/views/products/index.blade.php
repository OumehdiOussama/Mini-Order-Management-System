...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
    ...
    <div class="card group hover:shadow-card-hover transition-all duration-200 overflow-hidden flex flex-col">
        {{-- Product Image Placeholder --}}
        <div class="h-36 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center relative">
        ...
        <div class="p-4 flex flex-col flex-1">
            <a href="{{ route('products.show', $product) }}"
               class="text-sm font-semibold text-slate-800 dark:text-slate-200 hover:text-brand-600 dark:hover:text-brand-400 line-clamp-2 mb-1">
                {{ $product->name }}
            </a>
            <div class="mt-auto pt-3 flex items-center justify-between">
                ...
                <div class="flex gap-1">
                    <a href="{{ route('products.edit', $product) }}"
                       class="btn-icon text-slate-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20">
                    ...
                    <form method="POST" action="{{ route('products.destroy', $product) }}" class="inline">
                        ...
                        <button type="submit" data-confirm-delete="product"
                                class="btn-icon text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
